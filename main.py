...
fig, axes = plt.subplots(len(soil_params), 3, figsize=(15, len(soil_params) * 5))
plt.subplots_adjust(hspace=0.5)

for i, (param, group) in enumerate(top_wavelengths.groupby('Parameter')):
    selected_wavelengths = group['Wavelength'].tolist()
    
    for j, moisture in enumerate([0, 25, 50]):
        ...
        # Plot predictions for each moisture level and model
        ax = axes[i, j]  # Select subplot for current parameter and moisture level
        ax.scatter(y_test, y_pred, alpha=0.5)
        ax.plot([y_test.min(), y_test.max()], [y_test.min(), y_test.max()], 'r--')
        ax.set_xlabel("Actual Values")
        ax.set_ylabel("Predicted Values")
        ax.set_title(f"{param} at {moisture}ml Moisture Level")
        ax.grid(True)
